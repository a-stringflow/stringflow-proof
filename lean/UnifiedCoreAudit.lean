...
| `UnifiedCoreBridge.candidate_parameterization_of_reset_full_orbit_d` | proved | segment-length-`d` version: `g` is `d` steps before `x`, `e` is the weight into `g`; the input shape for `d=2/3/≥4` exclusions |
