...
| `UnifiedCoreBridge.d2_survivor_candidate_s0_odd` | proved | `s0 = x+1-5^(j-1)` is odd for the corrected `d=2` survivor; the document's "even `s0`" exclusion is invalid, and `d=2` is instead closed by the size bound below |
| `UnifiedCoreBridge.fullOrbitIter_upper_bound` | proved | all-`t=1` bound `3*2^n*r_n ≤ 22*5^n-2^n`; every exact full-orbit state is at most this trajectory |
| `UnifiedCoreBridge.d2_survivor_size_contradiction` | proved | for `n ≥ 5`, `17*r_n = 4*5^n-7` contradicts the upper bound: `(4*5^n-7)/17` is larger than the largest possible full-orbit state |
| `UnifiedCoreBridge.d2_exclusion_of_corrected_residue` | proved | corrected `d=2` survivor family is excluded for `j ≥ 6` by the size contradiction; `j=4` is the only small case and lies in the finite base |
...
| `UnifiedCoreBridge.fullOrbit_step_into_g` | proved | exact step `5*g_prev+1 = 2^e*g` from the full-orbit segment and incoming weight `e` |
...
| `UnifiedCoreBridge.candidate_parameterization_of_reset_full_orbit` | proved | packaged candidate bridge: `ResetHeadEq s0 (n0-1) k t δ r` + `5^k·s0=2^(e-1)*g+1` + exact full-orbit weights give `x = candidateX (n0-1) e g δ`, `x=fullOrbitIter (n0-1)`, `g=fullOrbitIter (n0-2)`, `e=orbitStepWeight (n0-3)` |
...
| `UnifiedCoreBridge.candidate_parameterization_of_reset_full_orbit_d` | proved | segment-length-`d` version with arbitrary `k`: `g` is `d` steps before `x`, `e` is the weight into `g`, terminal identity `5^k·s0=2^(e-1)*g+1`; packages the reset premises, while the `d≥2` candidate depth still needs the `n0-d` shift |
...
3. `unified_core_final_no_hge` now carries the explicit real-orbit
   ...
   position of `r_a` and the word shape from `r_j` to `r_a`; the `m2>0`
    ...
    `hr` is proved directly from the real-orbit segment by
    `previous_even_terminal_of_full_orbit_segment`:
    `r = (5*g_prev+1)/2 = 2^(e-1)*g`, with no general-orbit word
    decomposition.  The previous-terminal equation `s·5^k0 = r+1` is an
    explicit arithmetic premise of `hReset`, not a `GeneralOrbitFrom7`
    premise; the block-head reachability is `FullOrbitFrom7 r`.  The
    `k≥1` branch is excluded by `kge1_excluded_by_segment_step`:
    `k≥1` forces `x+1≡0 (mod 5)`, hence `x≡4 (mod 5)`, contradicting
    any `1+4a` step into `x`.  The remaining
    wiring is instantiating the `d≥2` candidate family from the real
    block word and wiring the `m2>0` tail exclusion.  The documented shortcut
    "`x+1 ≡ 22,56 (mod 32,64)` is prime to `5`" is invalid (`x ≡ 21
    (mod 32)` does not fix `x+1 mod 5`, e.g. `x=149`), but it is no
    longer needed for the bridge.  Second, the `m2>0` tail exclusion
    remains open: document 36.30.24's "even `s0`" exclusion is invalid
    (`s0` is odd, `d2_survivor_candidate_s0_odd`), but the `d=2` family
    is closed by the full-orbit size bound
    `d2_exclusion_of_corrected_residue`.  Until the word-decomposition
    wiring, the `m2>0` exclusion and the `d≥2` bridge enter the proof,
 ...
 `x ≡ 53 (mod 160)` / `x ≡ 183 (mod 320)` for the `e=3,j=17` branches and
 the size bound for the `d=2` survivor family.  Lean assembly into
